            ...
            <div class="product-card">
                ...
                <div class="product-info">
                    ...
                    <div class="product-price">
                        ${p.discount ? '<span class="old-price">' + p.price.toLocaleString() + ' KGS</span>' : ''}
                        <span class="current-price ' + (p.discount ? 'discounted' : '') + '">' + discountPrice.toLocaleString() + ' KGS</span>
                    </div>
                    <div style="font-size:0.8rem;color:' + stockColor + ';margin-bottom:10px;">' + stockText + '</div>
                    <button class="product-btn" onclick="handleAddToCartCatalog(' + p.id + ')" ' + (!inStock ? 'disabled style="background:#ccc;cursor:not-allowed;"' : '') + '>' + (inStock ? 'В корзину' : 'Нет в наличии') + '</button>
                </div>
            </div>
        `;
    }).join('');
    ...
    grid.innerHTML = filtered.map(p => {
        ...
        return `
            <div class="product-card">
                ...
                <div class="product-info">
                    <h3 class="product-title">${escapeHtml(p.name)}</h3>
                    <div class="product-price">${discountPrice.toLocaleString()} KGS</div>
                    <button class="product-btn" onclick="handleAddToCartCatalog(${p.id})" ${!inStock ? 'disabled style="background:#ccc;cursor:not-allowed;"' : ''}>${inStock ? 'В корзину' : 'Нет в наличии'}</button>
                </div>
            </div>
        `;
